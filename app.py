...
import cv2
from firearm_database import FirearmDatabase
...
def main():
    ...
    with tab5:
        st.header("🔫 Firearm Compatibility")
        
        # Initialize firearm database
        firearm_db = FirearmDatabase()
        
        # Create two columns for the main layout
        main_col1, main_col2 = st.columns([2, 1])
        
        with main_col1:
            st.subheader("Compatible Firearms")
            st.info(f"Showing firearms compatible with {caliber} caliber ammunition")
            
            # Get compatible firearms
            compatible_firearms = firearm_db.get_compatible_firearms(caliber)
            
            if compatible_firearms:
                # Create tabs for each compatible firearm
                firearm_names = [f['name'] for f in compatible_firearms]
                firearm_tabs = st.tabs(firearm_names)
                
                for idx, (firearm, tab) in enumerate(zip(compatible_firearms, firearm_tabs)):
                    with tab:
                        # Display firearm image
                        st.image(firearm['image_url'], 
                                caption=f"{firearm['name']} - {firearm['type']}", 
                                use_column_width=True)
                        
                        # Display firearm details
                        st.markdown(f"""
                        ### Specifications
                        - **Manufacturer:** {firearm['manufacturer']}
                        - **Type:** {firearm['type']}
                        - **Effective Range:** {firearm['effective_range']}
                ...
                st.warning(f"No firearms found for caliber {caliber}")
        
        with main_col2:
            st.subheader("Ballistic Data")
            
            # Get and display ballistic data
            ballistic_data = firearm_db.get_ballistic_data(caliber)
            
            if ballistic_data:
                st.markdown(f"""
                ### Technical Specifications
                - **Optimal Twist Rate:** {ballistic_data['optimal_twist_rate']}
                - **Typical Use Range:** {ballistic_data['typical_use_range']}
                - **Common Bullet Weights:** {ballistic_data['common_bullet_weights']}
                - **Recommended Barrel Length:** {ballistic_data['recommended_barrel_length']}
                
                ### Typical Applications
                """)
                
                for app in ballistic_data['typical_applications']:
                    st.markdown(f"- {app}")
                
                # Add current ammunition specs
                st.markdown("""
                ### Current Ammunition
                """)
                st.metric("Weight", f"{weight} grains")
                st.metric("Muzzle Velocity", f"{velocity} fps")
                
            else:
                st.warning(f"No detailed ballistic data available for caliber {caliber}")
            ...
            with st.expander("📋 Safety Information", expanded=False):
                ...
                4. Never point the firearm at anything you don't intend to shoot
                5. Store firearms and ammunition separately and securely
                
                *Consult a qualified instructor for proper firearm handling and safety training.*
                """)
